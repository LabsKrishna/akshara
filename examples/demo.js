#!/usr/bin/env node
// examples/demo.js — Interactive Database X demo
...
console.log = function (...args) {
  if (_muteEngine && typeof args[0] === "string" && args[0].startsWith("[dbx]")) return;
...
async function main() {
  _origLog("");
  _origLog(bold("  ╔══════════════════════════════════════════════════╗"));
  _origLog(bold("  ║          Database X — Live Demo                  ║"));
  ...
  _origLog(`      npm install dbx-memory`);
  _origLog(`      https://github.com/LabsKrishna/dbx`);
  _origLog("");
